"""Functions for words to be searched in PDF and txt files."""
import string
...
def extract_words(pdf_path, color):
    """Extract highlighted words with specified color in a PDF file.

    Args:
        pdf_path (str): target file path.
...
def scan_words(txt_path):
    ...
    words = [_remove_punctuation(w) for w in words]
    return words
